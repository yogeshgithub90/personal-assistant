...
public interface NotesHelper {
    static List<Note> createNotesFromRequest(@NotNull NotesServiceRequest request){
        return request.getNotes().stream().map(note -> Note.builder().title(note.getTitle()).userId(request.getUserId()).note(note.getNote()).keyword(note.getKeyword()).build()).collect(Collectors.toList());
    }
